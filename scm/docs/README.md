...
- **XDG-aware layered resolution** — searches `$XDG_CONFIG_DIRS`, `$XDG_CONFIG_HOME`, `$CONFIGBUILDER_CONFIG_DIR`, and explicit paths; later sources win at the key level
...
| Problem | Solution |
|---------|----------|
| Config loading tied to an application runtime | Standalone crate — no dependency on any runtime; library crates can load config without pulling one in |
...
| Diamond dep conflicts from multiple config-loading crates | One crate, published on crates.io — every consumer pins the same version |
