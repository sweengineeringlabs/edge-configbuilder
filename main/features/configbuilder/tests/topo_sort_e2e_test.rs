...
use swe_edge_configbuilder::__internal::topo_sort;
...
fn test_topo_sort_three_independent_nodes_returns_all_three_indices() {
    let order = topo_sort(&["x", "y", "z"], &[&[], &[], &[]]).unwrap();
...
fn test_topo_sort_linear_chain_dependency_loads_first() {
    // a(1) ← b(0): b needs a, so a must come before b in the result
    let order = topo_sort(&["b", "a"], &[&["a"], &[]]).unwrap();
...
fn test_topo_sort_diamond_graph_root_loads_before_leaf() {
    // a(3) is needed by b(1) and c(2); d(0) needs both b and c
    let names = &["d", "b", "c", "a"];
...
fn test_topo_sort_two_node_cycle_returns_err_with_cycle_in_message() {
    let err = topo_sort(&["a", "b"], &[&["b"], &["a"]]).unwrap_err();
    assert!(
        err.contains("cycle"),
...
fn test_topo_sort_self_cycle_single_node_returns_err() {
    let err = topo_sort(&["a"], &[&["a"]]).unwrap_err();
    assert!(err.contains("cycle"), "self-cycle must be detected: {err}");
}

#[test]
fn test_topo_sort_unknown_dependency_is_silently_ignored() {
    // "b" declares a dependency on "ghost" which is not in the names list
    let order = topo_sort(&["a", "b"], &[&[], &["ghost"]]).unwrap();
